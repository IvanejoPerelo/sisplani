...
export default function ModalDes() {
  ...
  const filtrarItems = () => {
    const filtrar = detailTable.filter((item) =>
      item.nombre.toLowerCase().includes(textBusqueda.toLocaleLowerCase)
    );
      ...
      <form className=" justify-between">
        <div className="mt-2">
          <table>
            ...
            <tbody>
              {global.length > 0 && global.map((item) => (
                <tr key={item.id}>
